...
def load_checkpoint(path, projector, emotion_head, emo_token, device):
    adapter_weights = load_file(os.path.join(path, "adapter_model.safetensors"))
    set_peft_model_state_dict(model, adapter_weights)
    projector.load_state_dict(torch.load(os.path.join(path, "projector.pt"), map_location=device))
    emotion_head.load_state_dict(torch.load(os.path.join(path, "emotion_head.pt"), map_location=device))
    emo_token.load_state_dict(torch.load(os.path.join(path, "emo_token.pt"), map_location=device))